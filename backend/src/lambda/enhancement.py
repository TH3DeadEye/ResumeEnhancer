...
def handler(event, context):
    ...
    try:
        ...
        return _response(500, {"error": f"Failed to load parsed resume: {str(e)}"})
...
- Authentic Scoring: Your assessment scores must be rigorous and reflect the genuine quality of the current resume. Do not inflate scores; if the resume is a 4/10 for ATS compatibility, score it a 4/10 and justify why.""",
        ...
        return _response(500, {"error": f"AI enhancement failed: {str(e)}"})
    ...
    try:
        ...
        return _response(500, {"error": f"Failed to save enhancement results: {str(e)}"})
    ...
    try:
        ...
        # Non-fatal — result was already saved to S3, so still return success
